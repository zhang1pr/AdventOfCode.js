...
function solve(input) {
  ...
  function compare(i, j) {
    ...
    let nsc = dsc.reduce((a, b, idx) => {
      let nidx = mapidx[idx];
      a[nidx] = sci[sami][nidx] + dsc[idx] * sign[nidx];
      return a;
    }, []);
  ...
  for (let [a1, b1, c1] of scanners) {
    for (let [a2, b2, c2] of scanners) {
      let a = Math.abs(a1 - a2), b = Math.abs(b1 - b2), c = Math.abs(c1 - c2);
