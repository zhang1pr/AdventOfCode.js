...
function B(input) {
  ...
  for (let group of arr) {
    ...
    max = Math.max(max, nums[0] * nums[2]);
    ...
    ogroup2.push([nums, sparr, damageType]);
  }
  ...
  while (true) {
    let group1 = cp(ogroup1), group2 = cp(ogroup2);

    for (let [nums] of group1)
      nums[2] += plus;
    ...
    while (group1.length && group2.length) {
      ...
      for (let [init, index, groupNum] of arr) {
        ...
        let kill = Math.floor(damage / hp2);
        enemy[enemyIndex][0][0] -= kill;
        cnt += kill;
      }
      ...
      if (cnt == 0 && group1.length && group2.length) break;
    }
