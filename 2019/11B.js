function B(input) {
  ...
  while(true) {
    ...
    boundaryX = Math.min(boundaryX, robotPosition[0]);
    boundaryY = Math.max(boundaryY, robotPosition[1]);
  ...
  panels.forEach((value, key) => {
    ...
    if (!array[boundaryY - y]) {
      array[boundaryY - y] = [];
    }
    array[boundaryY - y][x - boundaryX] = value;
  });
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    for (let j = 0; j < array[i].length; j++) {
      if (array[i][j] == null || array[i][j] === 'black') {
        array[i][j] = ' ';
      } else {
        array[i][j] = '#';
      }
